...
def _http_post(path, data=None, timeout=5):
    ...
    try:
        ...
        with _async_fire(req, timeout=timeout) as resp:
...
def schedule_page():
    # Accept POST from the form and delegate to schedule_arm
...
def _async_fire(path, data=None, timeout=None):
    try:
        if urllib is None:
            return False
        if timeout is None:
        ...
        _async_fire(req, timeout=timeout).read(1)
        return True
    except Exception:
        return False


def _async_fire(path, data=None):
    import threading
    def worker():
        try:
            app = _sched_app
...
def _cancel_timers():
    global _t_start, _t_stop
    try:
        if _t_start: _t_start.cancel()
    except Exception: pass
    try:
        if _t_stop: _t_stop.cancel()
    except Exception: pass
    _t_start = None
    _t_stop = None
