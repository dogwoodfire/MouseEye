...
try:
    serial = spi(port=SPI_PORT, device=SPI_DEVICE, gpio_DC=PIN_DC, gpio_RST=PIN_RST, bus_speed_hz=12000000)
    device = st7735(serial, width=WIDTH, height=HEIGHT, rotation=0, offset_left=2, offset_top=3)  # set rotation if needed
except Exception:
    sys.exit(0)
