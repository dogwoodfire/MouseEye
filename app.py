...
# Helpers to control the LCD systemd service (used around encoding)
def _lcd_is_active():
    try:
        p = subprocess.run(["systemctl", "is-active", "--quiet", "timelapse-lcd.service"],
                           stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=1.5)
        return (p.returncode == 0)
    except Exception:
        return False
...
def _lcd_service(action):
    # action should be 'stop' or 'start'
    try:
        subprocess.run(["sudo", "systemctl", action, "timelapse-lcd.service"],
                       stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=3)
    except Exception:
        pass
LCD_HIDE_SPLASH_FLAG = os.path.join(BASE, "lcd_hide_splash.flag")
...
# --- Helper: Rotate from src_path to dst_path atomically (for timelapse) ---
def _rotate_copy_to(src_path: str, dst_path: str, deg: int = None):
    """
    Read JPEG at src_path, rotate CCW (deg or UI policy), normalize EXIF,
    and atomically replace dst_path. Creates parent directory for dst if needed.
    """
    try:
        ...
        with Image.open(src_path) as im:
            ...
            if deg is None:
                # Reuse still policy (based on lcd_prefs.json)
                ui = _ui_deg()
                deg = 180 if ui == 90 else 270
            if deg % 360 != 0:
                im = im.rotate(deg, expand=True)
            exif = im.getexif()
            exif[ORIENT_TAG] = 1
...
def _start_encode_worker_once():
    ...
    def _encode_worker():
        while True:
            ...
            try:
                # If LCD service is active, instruct it to hide the splash and stop it
                try:
                ...
                if lcd_was_active:
                    ...
                    try:
                        ...
                        pass
                    # Stop the LCD service to free the device for encoding
                    _lcd_service("stop")
                ...
                # Prefer hardware encoder when available (bcm2835-codec / h264_v4l2m2m)
                hw_enc = "h264_v4l2m2m"
                use_hw = False
                try:
                    p = subprocess.run([FFMPEG, "-hide_banner", "-encoders"],
                                       stdout=subprocess.PIPE, stderr=subprocess.DEVNULL,
                                       text=True, timeout=2)
                    use_hw = (hw_enc in (p.stdout or ""))
                except Exception:
                    use_hw = False

                # Build ffmpeg command with conservative defaults; prefer hw encoder if present
                common = [
                    FFMPEG, "-y",
                    "-threads", "1",
                    "-framerate", str(fps),
                ...
                if use_hw:
                    # Use the Pi's hardware encoder if available. Keep bitrate explicit so quality is sane.
                    cmd = prio + common + [
                        "-vf", "scale='min(iw,1280)':'min(ih,1280)':force_original_aspect_ratio=decrease",
                        "-c:v", "h264_v4l2m2m",
                        "-b:v", "4000k",
                        "-maxrate", "4000k",
                        "-bufsize", "8000k",
                    ...
                    # Software fallback (libx264) — keeps good quality but is CPU intensive
                    cmd = prio + common + [
                        "-vf", "scale='min(iw,1280)':'min(ih,1280)':force_original_aspect_ratio=decrease",
                        "-c:v", "libx264",
                        "-preset", "veryfast",
                ...
                # Parse frame progress lines from ffmpeg's stdout if available
                while True:
...
<main>
{% set ap = ap_status %}
    <div class="card" id="ap-indicator" style="background:#e9f0f7;padding:5px 12px;">
  ...
  <div class="card session {% if current_session == s.name %}active{% endif %}">
    ...
    <div class="controls">
        ...
        <a class="btn" href="{{ url_for('download_session_zip', sess=s.name) }}" id="zip-download-{{ s.name }}" 
           style="margin-left:0px; display:{% if s.has_zip %}inline-flex{% else %}none{% endif %};">
           ⬇️ Zip file
        </a>
        </div>
...
(function(){
  ...
  function updateActive(st){
    ...
    if (end > 0) {
        ...
        if (remaining > 0) {
            timeEl && (timeEl.textContent = `• ${fmtDur(remaining)} left`);
            const total = Math.max(1, end - start);
            const elapsed = Math.max(0, now - start);
            const pct = Math.min(100, (elapsed / total) * 100);
            barEl && (barEl.style.width = pct + '%');
        } else {
            timeEl && (timeEl.textContent = '• Finishing...');
            barEl && (barEl.style.width = '100%');
        }
    } else {
        // Count-up for manual, indefinite sessions
        const elapsed = (start ? Math.max(0, now - start) : 0);
        timeEl && (timeEl.textContent = `• running ${fmtDur(elapsed)}`);
        barEl && (barEl.style.width = '100%'); // Progress bar is full for indefinite sessions
    }
...
(function() {
    ...
    async function poll() {
        ...
        // THE FIX: Check for ANY active zip job, just like we do for encoding.
        let anyZippingActive = false;
        ...
        if (jobs) {
            for (const [key, job] of Object.entries(jobs)) {
                ...
                if (key.startsWith('zip:') && (job.status === 'zipping' || job.status === 'queued')) {
                    anyZippingActive = true;
                }
                // Break early if both are found
                if (anyEncodingActive && anyZippingActive) break;
            }
        }
        
        const isJobActive = anyEncodingActive || anyZippingActive;

        // Update the global status text at the bottom
        const bgStatusEl = document.getElementById('background-status');
        if (bgStatusEl) {
            let statusText = '';
            if (anyEncodingActive) statusText = 'Status: Encoding...';
            else if (anyZippingActive) statusText = 'Status: Zipping...';
            bgStatusEl.textContent = statusText;
        }
        ...
        document.querySelectorAll('.session').forEach(card => {
            ...
            if (progEl && barEl) {
                if (encJob && (encJob.status === 'encoding' || encJob.status === 'queued')) {
            ...
            card.querySelectorAll('form[action*="/encode"] button, form[action*="/encode"] select').forEach(el => {
                // THE FIX: Disable if any job is active.
                el.disabled = isCaptureActive || isJobActive;
            });
            ...
            if (zipBtn) {
                // THE FIX: Disable if any job is active.
                zipBtn.disabled = isCaptureActive || isJobActive;
            }
            // This part remains specific to this card's zip job
